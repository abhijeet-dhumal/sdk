...
"""TransformersTrainer for HuggingFace Transformers and TRL with auto-instrumentation and checkpointing."""
...
class TransformersTrainer:
    """RHAI trainer for HuggingFace Transformers and TRL with auto-instrumentation and checkpoint support.
    ...
    Args:
        ...
        output_dir: Output directory for checkpoints. Supports pvc:// URIs for PVC mounting.
    ...
    def __post_init__(self):
        ...
        # Validate output_dir URI scheme
        if (
            self.output_dir
...
def get_trainer_cr_from_transformers_trainer(
    ...
    """Build Trainer CRD for TransformersTrainer with optional progression tracking and checkpointing.
    ...
    # Inject checkpoint code if enabled (before progression tracking)
    checkpoint_code = _build_checkpoint_code(trainer)
    if checkpoint_code:
        func_code = f"{checkpoint_code}\n\n{func_code}"

    # Wrap with progression tracking instrumentation if enabled (after checkpoint)
